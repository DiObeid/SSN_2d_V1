...
def find_rate(alphacen,alphas):
    ...
    rate=NR(C1)*exp(-(diffangcen**2.)/(2*sig_fori**2.))*fc(x,y) + NR(C2)*exp(-(diffangs**2.)/(2*sig_fori**2.))*fs(x,y) + 1e-8*Hz
    ...
    # #2- currents:
    # # average over time for each neuron 
...
if __name__ == "__main__":
 ...
 # lists and arrays to save the data:
 ...
 Sem_E=zeros((dim,dim))
 Sem_E[0,0]=0
 Sem_E[0,1:]=arange(-delta,delta+10,10)
 Sem_E[1:,0]=arange(delta,-delta-10,-10)

 Sem_I=zeros((dim,dim))
 Sem_I[0,0]=0
 Sem_I[0,1:]=arange(-delta,delta+10,10)
 Sem_I[1:,0]=arange(delta,-delta-10,-10)
 ...
 for angcen in alphacen:
  ...
  for angsur in alphas:
    response_e, response_i, sem_re, sem_ri= find_rate(angcen,angsur)
 ...
 savefitpath='/path/Center_Surround/'
 savefit='neuron_number_%.1f_pref_orientation_%.3f' % (recorded_neuron,theta[recorded_neuron])
 savefit.replace(' ','')

 fitname1= savefitpath+'Ang_E_Mtx_'+ savefit +'.txt'
 savetxt(fitname1,E_Mtx)
 
 fitname2= savefitpath+'Ang_I_Mtx_'+ savefit +'.txt'
 savetxt(fitname2,I_Mtx)

 fitname3= savefitpath+'Ang_Sem_E_'+ savefit +'.txt'
 savetxt(fitname3,Sem_E)

 fitname4= savefitpath+'Ang_Sem_I_'+ savefit +'.txt'
 savetxt(fitname4,Sem_I)

 

 # save_cur=[Sur_angle,save_e,save_i]
 # asave_cur=transpose(array(save_cur))

 # savi_cur=[Sur_angle,savi_e,savi_i]
 # asavi_cur=transpose(array(savi_cur))

 # savIn_cur=[Sur_angle,save_in,savi_in]
 # asavIn_cur=transpose(array(savIn_cur))

 # save_g=[Sur_angle,save_ge,save_gi]
 # asave_g=transpose(array(save_g))

 # savi_g=[Sur_angle,savi_ge,savi_gi]
 # asavi_g=transpose(array(savi_g))


 # fitname1='ST_Network_Inputs_to_Ecell_'+ savefit +'.txt'
 # savetxt(fitname1,asave_cur)

 # fitname2='ST_Network_Inputs_to_Icell_'+ savefit +'.txt'
 # savetxt(fitname2,asavi_cur)

 # fitname3='ST_Ext_Inputs'+ savefit +'.txt'
